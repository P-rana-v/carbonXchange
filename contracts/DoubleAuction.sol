...
// buyers set bid
function checkCreditsBeforePurchase(int32 _quantity) private view returns (bool) {
    // Calculate the total credits after the purchase
    int32 totalCredits = currentCredits + _quantity;

    // Calculate the maximum allowed credits after purchase (20% increase)
    int32 twentyPercentIncrease = (allocated * 120) / 100;

    // Check if the total credits after purchase exceed the allowed limit
    return totalCredits <= twentyPercentIncrease;
}

function BuyerBid(address _buyer, int32 _quantity, int32 _price) public payable {
    ...
    // Check if the purchase exceeds the 20% increase in credits
    require(checkCreditsBeforePurchase(_quantity), "Transaction exceeds 20% increase in credits.");
    ...
    // Update the current credits after a successful purchase
}
